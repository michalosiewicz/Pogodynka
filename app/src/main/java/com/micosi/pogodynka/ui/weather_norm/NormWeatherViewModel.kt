...
class NormWeatherViewModel(private val searchedCity: String) : ViewModel() {
    ...
    private suspend fun getCurrentWeather() {
        while (isFragmentOpen.value == true) {
            ...
            if (state is State.Success) {
                if (state.data != null) {
                    state.data.pressure.postValue("Ciśnienie : " + state.data.pressure.value)
                    state.data.east.postValue("Wschód słońca : " + state.data.east.value)
                    state.data.west.postValue("Zachód słońca : " + state.data.west.value)
